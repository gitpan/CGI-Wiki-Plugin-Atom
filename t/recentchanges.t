...
use_ok( "CGI::Wiki::Plugin::Atom" );

eval { my $atom = CGI::Wiki::Plugin::Atom->new; };
ok( $@, "new croaks if no wiki object supplied" );

eval {
        my $atom = CGI::Wiki::Plugin::Atom->new( wiki => "foo" );
     };
...
while ( ($store_name, $store) = each %stores ) {
  SKIP: {
      ...
      my $atom = eval {
          CGI::Wiki::Plugin::Atom->new( %default_config, site_url => "http://example.com/kakeswiki/" );
      };
      is( $@, "",
         "'new' doesn't croak if wiki object and mandatory parameters supplied"
      );
      isa_ok( $atom, "CGI::Wiki::Plugin::Atom" );
